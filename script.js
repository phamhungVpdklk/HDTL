document.addEventListener('DOMContentLoaded', () => {
    ...
    communeSelect.addEventListener('change', updateContractFields); updateContractFields();
    ...
    resultsContainer.addEventListener('input', (e) => {
        if (e.target.id === 'amount-paid-input') {
            ...
            const balance = invoiceData.grandTotal - amountPaid;
            ...
            invoiceData.amountOwed = amountOwed;
            invoiceData.refundAmount = refundAmount;
        }
    });
    exportLiquidationBtn.addEventListener('click', () => { if (!invoiceData.grandTotal && invoiceData.grandTotal !== 0) { alert("Chưa có dữ liệu để xuất."); return; } const printableHTML = generateLiquidationHTML(invoiceData); const printWindow = window.open('', '_blank'); printWindow.document.write(printableHTML); printWindow.document.close(); printWindow.focus(); });
    exportContractBtn.addEventListener('click', () => { if (!invoiceData.grandTotal && invoiceData.grandTotal !== 0) { alert("Chưa có dữ liệu để xuất."); return; } const printableHTML = generateContractHTML(invoiceData); const printWindow = window.open('', '_blank'); printWindow.document.write(printableHTML); printWindow.document.close(); printWindow.focus(); });
    ...
    function generateLiquidationHTML(data) {
        ...
        return `<html><head><title>Biên bản thanh lý hợp đồng</title><style> 
                        ...
                        .main-title { font-size: 16px; font-weight: bold; text-align: center; margin: 20px 0; } 
