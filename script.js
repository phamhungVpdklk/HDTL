document.addEventListener('DOMContentLoaded', () => {
    ...
    const exportAppointmentBtn = document.getElementById('export-appointment-btn');
    ...
    invoiceForm.addEventListener('submit', async (e) => {
        ...
        if (success) {
            ...
            currentRecordId = currentInvoiceData.id;
    ...
    function displayResultsOnPage(data) {
        ...
        exportAppointmentBtn.classList.remove('hidden');
    ...
    exportControlSheetBtn.addEventListener('click', () => {
        ...
        const phieuKiemSoatData = {
            ...
            diaChi: `${currentInvoiceData.contractInfo.location}, thành phố Long Khánh, Tỉnh Đồng Nai`,
    ...
    exportAppointmentBtn.addEventListener('click', () => {
        if (!currentInvoiceData || !currentInvoiceData.grandTotal && currentInvoiceData.grandTotal !== 0) { alert("Chưa có dữ liệu để xuất."); return; }
        const printableHTML = generateAppointmentSlipHTML(currentInvoiceData);
        printDocument(printableHTML);
    });
    ...
    function clearForm(shouldConfirm) {
        if (shouldConfirm && !confirm("Hành động này sẽ xóa toàn bộ thông tin đã nhập trên biểu mẫu. Bạn có chắc không?")) return;
