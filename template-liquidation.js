function generateLiquidationHTML(data) {
    ...
    data.allOptionalItems.forEach(parcel => {
        parcel.services.forEach(service => {
            const unitText = service.unit === 'm²' ? 'm<sup>2</sup>' : service.unit;
             // THAY ĐỔI: Thêm địa chỉ thửa đất (parcel.address) vào phần mô tả
            const description = service.type === 'area_based'
                ? `${service.name} (Tờ ${parcel.mapSheet}, Thửa ${parcel.parcelNo}, Dt ${parcel.area} m², tại ${parcel.address})`
                : `${service.name} (Tờ ${parcel.mapSheet}, Thửa ${parcel.parcelNo}, tại ${parcel.address})`;
    ...
    return `<html><head><title>Biên bản thanh lý hợp đồng</title><style> 
            @page {
                size: A4 portrait;
                margin-left: 1.5cm;
                margin-top: 1cm;
            ...
            body { font-family: 'Times', serif; font-size: 12pt; line-height: 1.3; color: #000; } 
            .bold { font-weight: bold; } .italic { font-style: italic; } 
            .header-table { width: 100%; border-collapse: collapse; font-size: 11pt; table-layout: fixed;} 
            ...
            .main-title { font-size: 14pt; font-weight: bold; text-align: center; margin: 20px 0; } 
            .content p { text-indent: 30px; margin: 2px 0; text-align: justify; } 
            ...
            .cost-table th, .cost-table td { border: 1px solid #333; padding: 3px 4px; text-align: left; vertical-align: top; font-size: 11pt; line-height: 1.2; } 
            ...
            <div class="footer"> 
                <table class="footer-table"> 
                    ...
                    <tr> 
                        <td><strong class="bold">${data.clientInfo.name}</strong></td> 
                        <td><strong class="bold">Phạm Văn Hải</strong></td> 
                    </tr> 
                </table> 
            </div>
        </body></html>`;
